...
export async function dbGetOrgByInvite(code: string) {
  ...
  const res = await fetch(
    `${backendUrl}/api/organizations?invite_code=${encodeURIComponent(code)}`
  );
...
export async function dbLoginOrg(email: string, password: string) {
  const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || '';
  const res = await fetch(
    `${backendUrl}/api/organizations?admin_email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`
  );
